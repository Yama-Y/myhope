...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			if($(':input[name="data.id"]').val() == '${sessionScope.sessionInfo.user.id}'){
				url = myhope.contextPath + '/base/user!doNotNeedSessionAndSecurity_update.myhope';
			}else{
	...
	$(function() {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(myhope.contextPath + '/base/user!getById.myhope', {
				...
				if (result.id != undefined) {
					...
					if (result.photo) {
						$('#photo').attr('src', myhope.contextPath + result.photo);
					}
				}
		...
		uploader = new plupload.Uploader({//上传插件定义
			...
			url : myhope.contextPath + '/plupload?fileFolder=/userPhoto',//上传文件路径
	...
	<form method="post" class="form">
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<tr>
					<th>qq</th>
					<td><input name="data.qq" /></td>
					<th>邮箱</th>
					<td><input name="data.mail" /></td>
				</tr>
				<tr>
					<th>照片上传</th>
					<td><div id="container">
